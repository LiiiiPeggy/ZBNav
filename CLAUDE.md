...
<!-- ################################ -->
<!-- Markdown: SLAM workspace now contains only odin_ros_driver -->
<!-- ################################ -->
# 3. SLAM — odin_ros_driver (sole package in this workspace)
cd SLAM
...
> `4savemap.sh` (save the `.bin` map while the driver is in SLAM mode), `5bin2ply.sh` (`.bin`→`.ply`),
> `6ply2pcd.sh` (`.ply`→`.pcd` VoxelGrid downsample), `showmap.sh` (standalone saved-map viewer —
> its own `/overall_map` publisher + `overall_map.rviz`; do NOT run alongside `3run_relocalization.sh`;
> they would both publish `/overall_map`).
...
```
Odin 自带 SLAM (`odin_ros_driver`) ──remap──▶ /state_estimation, /registered_scan ──▶ planner
        │
        └─ registered_scan_adapter_node (PointXYZRGB → PointXYZI + min-range filter)
```

<!-- ################################ -->
<!-- Markdown: Odin is the sole SLAM backend after super_lio removal -->
<!-- ################################ -->
`registered_scan_adapter_node` (in `odin_ros_driver`) converts Odin's `PointXYZRGB` `/odin1/cloud_slam` into the `PointXYZI` format CMU planner expects (drops rgb, sets `intensity=0`, filters points closer than `scan_min_range` from the vehicle via `/state_estimation`), publishing to `/registered_scan`.
...
<!-- ################################ -->
<!-- Markdown: SLAM workspace now holds only odin_ros_driver -->
<!-- ################################ -->
- **`SLAM`** — Core SLAM system (single package):
...
| `/velodyne_points` | `rs_converter` | — (no in-repo consumer since super_lio removal) |
| `/livox/lidar` (CustomMsg) | Livox driver | `lidar_imu_init` |
| `/state_estimation` | `odin_ros_driver` (via remap) | `local_planner`, `terrain_analysis`, `terrain_analysis_ext`, `waypoint_example`, `waypoint_rviz_plugin`, `visualization_tools`, `cruiseController` |
| `/registered_scan` | `odin_ros_driver` → `registered_scan_adapter_node` | `local_planner`, `terrain_analysis`, `terrain_analysis_ext`, `visualization_tools` |
